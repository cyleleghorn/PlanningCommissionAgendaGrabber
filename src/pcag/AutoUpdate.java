...
public class AutoUpdate 
{
	...
	private static String[] currentVersion=MainGUI.version.split("\\.");
	...
	public static void checkForUpdates(String callBackLocation)
	{
		Runnable r = new Runnable()
		{
			...
			public void run() 
			{
				...
				if(!Arrays.equals(remoteVersion, new String[]{"0","0","0"}) && Arrays.equals(remoteVersion, MainGUI.version.split("\\.")))
				{
					...
					{
						JOptionPane.showMessageDialog(MainGUI.mainFrame, "You have the latest version!", "Up to date", JOptionPane.INFORMATION_MESSAGE);
					}
				}
				else if(!Arrays.equals(remoteVersion, new String[]{"0","0","0"}) && !Arrays.equals(remoteVersion, MainGUI.version.split("\\.")))
				{
					if(needsUpdate())  //Actually check to see if the remoteVersion is bigger.
					{
						...
						int updateOrNah = JOptionPane.showConfirmDialog(MainGUI.mainFrame, "There is a new version available! Do you wish to update?\nYour version: V" + MainGUI.version + "\nLatest version: V" + remoteVersion[0] + "." + remoteVersion[1] + "." + remoteVersion[2], "Please Update", JOptionPane.YES_NO_OPTION);
	...
	private static void update()
	{
		...
		try {
			...
			JOptionPane.showMessageDialog(MainGUI.mainFrame, "Download failed!", "Error", JOptionPane.ERROR_MESSAGE);
			e.printStackTrace();
		}
	...
	private static void createBatch()
	{
		...
		try {
			...
			File dir = new File(MainGUI.class.getProtectionDomain().getCodeSource().getLocation().getPath());
